 ...
 python Export.py --username 'barackobama' --since 2015-09-10 --until 2015-09-12 --maxtweets 1\n"""
	...
	try:
		...
		for t in got.manager.TweetManager.getTweets(tweetCriteria):
			outputFile.write('\n{"user": %s, "created_at": %s, "retweets": %d, "favorites": %d, "text": "%s", "geo": %s, "mentions": %s, "hashtags": %s, "id": %s, "permalink": %s}' % (t.username, t.date.strftime("%Y-%m-%d %H:%M"), t.retweets, t.favorites, t.text, t.geo, t.mentions, t.hashtags, t.id, t.permalink))
